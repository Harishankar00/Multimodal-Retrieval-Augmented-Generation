...
class LLMService:
    def query_llm(self, query: str, retrieved_blocks: List[OCRBlock], page_images: List[Image.Image]) -> str:
        ...
        if not settings.LLM_API_KEY:
            return "⚠️ LLM Configuration Error: LLM_API_KEY environment variable is not set. Please create a backend/.env file and define it to enable VQA."
...
Answer:"""
        ...
        try:
            ...
            if response.status_code != 200:
                return f"⚠️ LLM API Error: Request failed with status code {response.status_code}.\n\nDetails: {response.text}"
                
            response_data = response.json()
            if "choices" in response_data and len(response_data["choices"]) > 0:
                return response_data["choices"][0]["message"]["content"]
            else:
                return f"⚠️ LLM API Response Error: Received unexpected payload format: {str(response_data)}"
                
        except Exception as e:
            return f"⚠️ LLM API Connection Error: Failed to connect to the model provider endpoint.\n\nDetails: {str(e)}"
